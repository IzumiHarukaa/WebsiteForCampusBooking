...
public class BookingController {
     ...
     * Shows both the immutable audit trail AND live booking status for
     * requests this approver has already processed (i.e. bookings whose
     * status has moved past the approver's level in the chain).
     */
    ...
    public String showApproverHistory(Model model, Principal principal,
        ...
        String authority = authentication.getAuthorities().iterator().next().getAuthority();
        String roleName = switch (authority) {
            case "ROLE_STAFF_ADVISOR" -> "Staff Advisor";
            case "ROLE_HOD" -> "HOD";
            case "ROLE_DEAN" -> "Dean";
            case "ROLE_PRINCIPAL" -> "Principal";
        ...
        // 1. Immutable audit trail snapshots (existing behaviour)
        ...
        // 2. Live bookings this approver has already acted on
        //    (status is past their level OR rejected by anyone)
        List<Booking> processedBookings = bookingService
                .findProcessedBookingsForApprover(person.getId(), roleName);
        model.addAttribute("processedBookings", processedBookings);
    ...
    /**
     * Role-aware dashboard redirect. Sends the approver back to their
     * specific dashboard so the "Dashboard" button in the history page
     * doesn't accidentally log them out.
     */
    @GetMapping("/approver/dashboard")
    public String approverDashboardRedirect(org.springframework.security.core.Authentication authentication) {
        String authority = authentication.getAuthorities().iterator().next().getAuthority();
        return switch (authority) {
            case "ROLE_STAFF_ADVISOR" -> "redirect:/staff-advisor/dashboard";
            case "ROLE_HOD" -> "redirect:/hod/dashboard";
