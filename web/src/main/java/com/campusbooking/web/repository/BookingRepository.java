...
public interface BookingRepository extends JpaRepository<Booking, String> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT b FROM Booking b " +
            "JOIN StudentApproverMapping map ON b.user.id = map.student.id " +
            "WHERE map.staffAdvisor.id = :approverId")
    List<Booking> findProcessedByStaffAdvisor(@org.springframework.data.repository.query.Param("approverId") Long approverId);

    @org.springframework.data.jpa.repository.Query("SELECT b FROM Booking b " +
            "JOIN StudentApproverMapping map ON b.user.id = map.student.id " +
            "WHERE map.hod.id = :approverId")
    List<Booking> findProcessedByHOD(@org.springframework.data.repository.query.Param("approverId") Long approverId);
